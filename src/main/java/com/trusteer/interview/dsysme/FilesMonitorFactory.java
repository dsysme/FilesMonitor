 ...
 * For each monitored file it creates the HttpFileTrackingInfo that will serve as a baseline for checking modifications
...
public enum FilesMonitorFactory {
    ...
    public FilesMonitor createNewFilesMonitor(List<HttpFileDescriptor> filesToMonitor) {
        Map<HttpFileDescriptor, HttpFileTrackingInfo> trackingInfoMap = new HashMap<>();
        for (HttpFileDescriptor file : filesToMonitor) {
            try {
                ...
                trackingInfoMap.put(file, new HttpFileTrackingInfo(hash));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
